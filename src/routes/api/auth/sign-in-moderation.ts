...
import { z } from "zod";
import { getActiveBanMatch } from "#/features/admin/server/admin-bans-repository";
		...
		handlers: {
			POST: async ({ request }) => {
				...
				try {
					...
					const ban = await getActiveBanMatch({
					...
					return Response.json(
						{ success: false, error: "Unable to check account status." },
						{ status: 500 },
					);
				}
			},
