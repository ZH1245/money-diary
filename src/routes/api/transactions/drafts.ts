...
import {
  ...
  getUserDraftTransactions,
  isCategoryAccessibleByUser,
} from '#/features/transactions/server/transactions-repository'
import { createScheduledTransactionSchema } from '#/features/transactions/schemas/transaction'
  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const rows = await getUserDraftTransactions(userContext.id)
      ...
      POST: async ({ request }) => {
        ...
        if (!parsed.success) {
          return Response.json(
            { success: false, error: 'Invalid scheduled transaction payload', details: parsed.error.flatten() },
            { status: 400 },
          )
        }
        ...
        const categoryId = resolveTransactionCategoryId(parsed.data.type, parsed.data.categoryId ?? null)

        if (requiresTransactionCategory(parsed.data.type) && categoryId === null) {
          return Response.json({ success: false, error: 'Category is required for expense' }, { status: 400 })
        }
