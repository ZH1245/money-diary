...
function SignUpPage() {
	...
	function getFieldError(
		field: "name" | "email" | "password" | "currency",
		value: string,
	): string {
			...
			formPanel={
				<article className="rise-in w-full max-w-[340px] rounded-panel border border-border bg-panel p-6 shadow-sm sm:p-7">
					...
					<p className="mt-2 text-sm text-muted-foreground">
						Start your money tracking workspace.
					</p>
						...
						<div>
							<label className="flex items-start gap-2 text-xs leading-relaxed text-muted-foreground">
								<input
									...
									disabled={isSubmitting}
									className="mt-0.5 size-4 rounded border border-border accent-[var(--accent)]"
								/>
								<span>
									I agree to the{" "}
									<Link
					...
					features={[
						{
							...
							description:
								"Type “Netflix 1,200” or “salary 80k” — AI adds the right transaction for you.",
						},
						{
							icon: BarChart3,
							title: "Ask anything about your money",
							description:
								"“What did I spend on food this month?” or “Show my top categories” — answered from your data.",
						},
						{
							icon: Sparkles,
							title: "Savings, goals, and wishlist too",
							description:
								"Create savings entries, goals, and wishlist items without filling long forms.",
						},
					]}
				/>
			}
