...
function buildAiTestPayload({
	...
	if (providerId === "gemini") {
		...
		if (savedApiKeyMask)
			return { provider: "gemini" as const, useStoredKey: true as const };
		return null;
	}
...
export function AiSettingsSection() {
	...
	const [globalAiStatus, setGlobalAiStatus] = useState<
		AiSettingsResponse["global"] | null
	>(null);
	...
	const selectedProvider = useMemo(
		() =>
			AI_PROVIDER_OPTIONS.find((provider) => provider.id === providerId) ??
		...
		async (nextProviderId: string, nextBaseUrl: string, nextApiKey: string) => {
			...
			try {
				...
				if (!response.ok || !payload?.success || !payload.data) {
					throw new Error(
						payload?.error ?? "Unable to test AI provider connection",
					);
				}
	...
	async function handleAiSourceChange(nextUseGlobal: boolean) {
		...
		try {
			...
			setUrlProbe(INITIAL_URL_PROBE);
			toast.success(
				nextUseGlobal ? "Using app AI service" : "Using your own AI provider",
			);
		} catch (error) {
			setErrorMessage(
				error instanceof Error ? error.message : "Unable to update AI source",
			);
		} finally {
	...
	async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
		...
		const requestPromise = fetch("/api/settings/ai", {
			...
			if (!response.ok || !payload?.success) {
				...
				throw new Error(
					fieldError ??
						payload?.details?.formErrors?.at(0) ??
						payload?.error ??
	...
	async function handleRemoveApiKey() {
		...
		const confirmed = window.confirm(
			"Remove the stored API key? You will need to enter it again before using providers that require one.",
		);
		if (!confirmed) return;
		...
		<article className="md-panel p-5 md:p-6">
			<div className="flex flex-wrap items-start justify-between gap-3">
				<div>
					...
					<p className="mt-1 text-xs text-muted-foreground">
						Ollama, Google Gemini, and OpenRouter are live. Other providers are
						listed for upcoming support.
					</p>
				</div>
				<span className="rounded-full bg-soft-accent px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide text-income">
					Live
				</span>
			</div>
			...
			<form className="mt-4 space-y-4" onSubmit={handleSubmit}>
				{globalServiceConfigured ? (
					<fieldset className="space-y-3 rounded-lg border border-border-faint bg-input-bg p-4">
						...
						<label className="flex cursor-pointer items-start gap-3 text-sm">
							...
							<span>
								...
								<span className="mt-0.5 block text-xs text-muted-foreground">
									Provider: {globalAiStatus?.provider ?? "—"} · Model:{" "}
									{globalAiStatus?.model ?? "—"}
								</span>
								{!globalServiceEnabled ? (
					...
					<p className="rounded-md border border-border-faint bg-input-bg px-3 py-2 text-xs text-muted-foreground">
						No app AI service is configured yet. Set up your own provider below.
					</p>
				)}

				{settingsWarning ? (
					<p className="rounded-md border border-expense/40 bg-soft-accent px-3 py-2 text-xs text-expense">
						{settingsWarning}
					</p>
				) : null}
				...
				{showCustomSettings ? (
					<>
						...
						{providerId === "ollama" ? (
							<>
								...
								{urlProbe.message ? (
									...
									>
										{urlProbe.message}
										{urlProbe.statusCode != null
											? ` (HTTP ${urlProbe.statusCode})`
											: null}
									</p>
								) : null}
								{urlProbe.status === "failed" ? (
									<p className="-mt-2 text-xs text-muted-foreground">
										The URL could not be reached. You can still save it if
										Ollama is offline right now or only reachable from another
										network.
									</p>
								) : null}
								<FormField
						...
						{isProviderEnabled ? (
							<p className="text-xs text-muted-foreground">
								Sensitive values are encrypted with ENV_SECRETS before storage.
								Saved keys show in masked form.
							</p>
						) : null}
						...
						<button
							...
							className={`inline-flex h-10 items-center justify-center gap-2 rounded-md px-4 text-sm font-medium text-primary-foreground transition-all duration-200 hover:brightness-110 active:scale-[0.99] disabled:opacity-60 ${
								urlProbe.status === "failed" ? "bg-expense" : "bg-primary"
							}`}
						>
					...
					<p className="text-xs text-muted-foreground">
						You are using the app AI service. Switch to your own provider above
						if you want custom credentials.
					</p>
				)}

				{errorMessage && !showCustomSettings ? (
