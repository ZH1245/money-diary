...
export function AdminGlobalAiSection() {
		...
		async function loadSettings() {
			...
			try {
				...
				if (!response.ok || !payload?.success) {
					throw new Error(
						payload?.error ?? "Unable to load global AI settings",
					);
				}
				...
				setErrorMessage(
					error instanceof Error
						? error.message
						: "Unable to load global AI settings",
				);
			} finally {
				setIsLoading(false);
			}
	...
	async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
		...
		try {
			...
			setErrorMessage(
				error instanceof Error
					? error.message
					: "Unable to save global AI settings",
			);
		} finally {
	...
	async function handleRemoveApiKey() {
		...
		setErrorMessage(null);
		try {
			const response = await fetch("/api/admin/global-ai/key", {
				method: "DELETE",
			});
			const payload = (await response.json().catch(() => null)) as {
				success?: boolean;
			...
			<div className="flex flex-wrap items-start justify-between gap-3">
				<div>
					...
					<p className="mt-1 text-xs text-muted-foreground">
						Configure the AI service users can use by default. API keys are
						encrypted and never shown to regular users.
					</p>
				</div>
			</div>
