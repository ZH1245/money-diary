...
export function AdminGlobalCategoriesSection() {
		...
		try {
			...
			setErrorMessage(
				error instanceof Error
					? error.message
					: "Unable to load global categories",
			);
		} finally {
			setIsLoading(false);
		}
	...
	const slugPreview = useMemo(
		() => slugifyCategoryName(form.name.trim()),
		[form.name],
	);
				...
				cell: ({ row }) => (
					<div className="flex justify-end">
						<DeleteRowButton
							...
							onConfirm={() =>
								void handleDeleteCategory(row.original.id, row.original.name)
							}
						/>
					</div>
				),
			...
			<div className="flex flex-wrap items-center justify-between gap-3">
				<div>
					...
					<p className="mt-1 text-xs text-muted-foreground">
						Built-in categories appear for every user. Users can still create
						personal categories.
					</p>
				</div>
				<Button
			...
			<Sheet open={isSheetOpen} onOpenChange={handleSheetOpenChange}>
				<SheetContent className="w-full sm:max-w-md">
					<SheetHeader>
						...
						<SheetDescription>
							This category will be visible to all users as a built-in option.
						</SheetDescription>
					</SheetHeader>
						...
						<div className="grid gap-2">
							...
							<Select
								...
								onValueChange={(value) =>
									setForm((previous) => ({
										...previous,
										kind: value as CategoryFormState["kind"],
									}))
								}
								disabled={isSubmitting}
							>
